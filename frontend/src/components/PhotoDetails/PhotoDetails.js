...
function PhotoDetails(props) {
    ...
    useEffect(() => {
        const getPhoto = async function(){
            const res = await fetch("http://" + process.env.REACT_APP_IP_ADDY + ":3001/photos/" + props.photoId)
    ...
    const postComment = async (e) => {
        ...
        const res = await fetch("http://" + process.env.REACT_APP_IP_ADDY + ":3001/comments?photoId=" + props.photoId, {
            method: "POST",
            credentials: "include",
            headers: { 
    ...
    return(
        <div className={styles['container']}>
            <div className={styles['image']}>
                {photo && <img src={"http://" + process.env.REACT_APP_IP_ADDY + ":3001/"+photo.path} alt={photo.name}/>}
            </div>
            <div className={styles['comment-section']}>
                ...
                <div className={styles['submit-comment']}>
                    <form onSubmit={postComment} className={styles['comment-form']}>
                        ...
                        <i className={`fa-regular fa-paper-plane icon ${comment ? 'icon-secondary' : ''} ${styles['like-icon']}`} title="Post comment"/>
                        </button>
                    </form>
                </div>)}
