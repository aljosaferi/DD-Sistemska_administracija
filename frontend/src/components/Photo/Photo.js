...
import { faHeart, faComments, faFlag } from "@fortawesome/free-regular-svg-icons";
...
function Photo(props){
    ...
    const reportPhoto = async () => {
        ...
        fetch(`http://localhost:3001/photos/${props.photo._id}/flag`, {
                method: 'POST',
                credentials: 'include',
                headers: {
                    'X-CSRF-Token': CSRFtoken.CSRFToken
            ...
            <div className={styles['interactions']}>
                <div className={styles['like-comment-report']}>
                    <div className={styles['like-comment']}>
                        <FontAwesomeIcon icon={hasLiked ? faSolidHeart : faHeart} className={`icon ${hasLiked ? 'icon-secondary' : ''} ${styles['like-icon']}`} onClick={likePhoto} title="Like"/>
                        <FontAwesomeIcon icon={faComments} className={`icon ${styles['comments-icon']}`} onClick={openImageModal} title="Comments"/>
                    </div>
                    <FontAwesomeIcon icon={faFlag} className={`icon ${styles['report-icon']}`} onClick={reportPhoto} title="Report"/>
                </div>
